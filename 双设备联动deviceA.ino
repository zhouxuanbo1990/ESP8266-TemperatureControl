...
// 静态 IP 配置
const IPAddress local_ip(192, 168, 2, 3);
...
bool lastPeerOnlineState = false;  // 新增，修复编译错误
...
void tryConnectMQTT(bool forceAttempt) {
  ...
  if (client.connect(clientId.c_str(), mqtt_user, mqtt_password, deviceA_status_topic, 0, false, "OFF")) {
    ...
    lastMqttConnectedState = true; // 立即同步状态，防止loop中重复打印“恢复”
...
void checkTemperature() {
  ...
  if (tempSensorFailed) {
    if (millis() - lastRelayChange >= relayDebounceTime) {
      ...
      Serial.println(F("温度传感器故障，已停止水泵"));
    }
    return;
  }
...
void setup() {
  ...
  // 无论初始WiFi是否成功，都初始化服务以便后续重连后直接工作
  initWebServer();
  setupMQTT();
  if (WiFi.status() == WL_CONNECTED) {
    tryConnectMQTT(true);
...
void loop() {
  ...
  if (tempSensorFailed) {
    if (millis() - ledTimer > 200) {
      ledTimer = millis();
      ledState = !ledState;
      digitalWrite(PIN_LED, ledState ? LOW : HIGH);  // 快速闪烁
    }
  } else if (WiFi.status() != WL_CONNECTED) {
    if (millis() - ledTimer > 500) {
      ledTimer = millis();
      ledState = !ledState;
      digitalWrite(PIN_LED, ledState ? LOW : HIGH);  // 慢速闪烁
    }
  } else {
  ...
  // 移除每60秒一次的冗余周期检查，checkPeerStatus 已经提供了更精准的检测
